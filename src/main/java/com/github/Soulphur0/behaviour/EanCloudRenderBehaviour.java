...
public class EanCloudRenderBehaviour {
  ...
  public static void ean_renderClouds(WorldRenderer instance, MatrixStack matrices, Matrix4f projectionMatrix,
    ...
    // * The geometry for each layer is built using its own parameters, and is
    // stored in an array.
    // * All values besides the geometry that are important for rendering are stored
    // in an array too.
    for (int layerNum = 0; layerNum < config.getNumberOfLayers(); layerNum++) {
      ...
      layer.setRenderAltitude((float) (cloudRenderAltitude / layer.getCloudThickness()
          - (double) MathHelper.floor(cloudRenderAltitude / layer.getCloudThickness())) * layer.getCloudThickness());
      ...
      // Maybe used to position the pivot of the clouds within a relative position to
      // the player's camera.
      ...
      // Used to translate the clouds little by little, a fraction of their position
      // at a time.
      layer.setTranslationX((float) (l - (double) MathHelper.floor(l)));
      layer.setTranslationZ((float) (n - (double) MathHelper.floor(n)));

      // ; Multiply world renderer's cloud color with the user chosen color vector,
      ...
      layer.setVertexGeometry(
          ean_preProcessCloudLayerGeometry(layer, bufferBuilder, l, cloudRenderAltitude, n, colorVec3d)); // > Cloud
                                                                                                          // geometry
                                                                                                          // building.
      ...
      // TODO Find out what these parameters do... this might have to do with the
    ...
    // * Using the previously generated array, clouds are rendered with their own
    // settings.
    for (CloudLayer layer : CloudConfig.cloudLayers) {
      ...
      if (layer.getVertexGeometry() != null) {
        ...
        var h = RenderSystem.getShaderFogColor();
        RenderSystem.setShaderFogColor(0, 0, 0, 0);
        // RenderSystem.setShaderFogStart(0.0f);
        // RenderSystem.setShaderFogEnd(6f);
        // RenderSystem.setShaderFogShape(FogShape.SPHERE);

        // BackgroundRenderer.applyFogColor();// test this later
  ...
  private static BufferBuilder.BuiltBuffer ean_preProcessCloudLayerGeometry(CloudLayer layer,
  ...
  private static void ean_buildCloudLayerGeometry(CloudLayer layer, BufferBuilder builder, double x, double y, double z,
      Vec3d color) {
    ...
    // > A texture displacement in pixels has to be added in order to not render the
    // same part of the clouds texture on consecutive layers.
    // < It is best to displace the texture in a single direction; I arbitrarily
    // chose the EAST.
    ...
    // - The value of these counters used in the 'for' loop below determine the span
    // in quadrants of cloud rendering in cardinal directions.
    // / They also determine (in quadrants) the region of the clouds texture to
    // render.
    // / The minimal limits are -1 0, which centers the clouds above the player with
    // a render distance of exactly 4 chunks.
    // / To properly center clouds, both limits must increment by 1, so render
    // distance can only increase by 2 chunks at a time at minimum.
    int westToEastSpan; // Lower negative limit => greater West render. Higher positive limit => greater
                        // East render.
    int northToSouthSpan; // Lower negative limit => greater North render. Higher positive limit =>
                          // greater South render.
    ...
    if ((layer.getCloudType().equals(CloudConfig.CloudTypes.FANCY) && layer.isWithinRenderDistance())
        || (layer.getCloudType().equals(CloudConfig.CloudTypes.LOD) && layer.isWithinLodRenderDistance())) {
      // + The following 'for' loop counters determine cloud render distance.
      // * Much like the Minecraft world, cloud are rendered in chunks that I will
      // call 'quadrants'.
      // * Cloud quadrants are made out 8x8 flat blocks. One quadrant would equal 1
      // single iteration of this loop.
      // * A flat block is in reality a single pixel of the clouds texture inflated
      // into a cloud.

      // + Pixel displacement should be done in multiples of 8.
      // * The displacement value is added to the drawing position of each vertex, for
      // every 8 pixels of displace,
      // * 1 quadrant is subtracted from the westToEast variable, which determines the
      // square region of the texture to draw.
      for (westToEastSpan = northwestSpan - displacementInQuadrants; westToEastSpan <= southeastSpan
          - displacementInQuadrants; ++westToEastSpan) {
        for (northToSouthSpan = northwestSpan; northToSouthSpan <= southeastSpan; ++northToSouthSpan) {

          // - The value of these variables determine where to draw clouds in the
          // previously set render distance.
          ...
          // * The float values added to the drawing positions of the vertex determine the
          // pixel of the clouds.png texture positions to be drawn.
          // * Clouds are drawn in square groups of 8x8 pixels that I called "quadrants"
          // (they are cloud-chunks in a way) [(0,8);(8,8);(8,0);(0,0)].
          // * Higher or lower values make clouds split or overlap. The size of quadrants
          // are determined by the previous variables.
          // - If we want to add a displacement to the cloud layer; we first need for the
          // loop variables to be located in that part of the texture before we try to
          // draw said part of the texture.
          // / Since here we are drawing quadrants, 1 unit up there in the loop is equal
          // to 8 pixels.
          // / For a displacement of 100 pixels, we would need 100/8 quadrants; that's why
          // displacement should be set in multiples of 8.
          // = So, in order to displace by 1 (quadrant) the cloud layer, horizontal
          // displacement should be = 8 and westToEastSpan should be -= 1 for both of its
          // ends.

          // ? This renders the bottom face of clouds.
          if (ab > -8.0F) {
            // The added values to each vertex should be modified all at once and by the
            // same amount; changing a single value makes it draw in the specified way only
            // when looking at the direction of that vertex.
            builder
                .vertex((double) (westToEastDrawPos + 0.0F + textureDisplacement), (double) (ab + 0.0F),
                    (double) (northToSouthDrawPos + 8.0F))
                .texture((westToEastDrawPos + 0.0F) * 0.00390625F + k, (northToSouthDrawPos + 8.0F) * 0.00390625F + l)
                .color(s, t, u, layer.getCloudOpacity()).normal(0.0F, -1.0F, 0.0F).next();
            builder
                .vertex((double) (westToEastDrawPos + 8.0F + textureDisplacement), (double) (ab + 0.0F),
                    (double) (northToSouthDrawPos + 8.0F))
                .texture((westToEastDrawPos + 8.0F) * 0.00390625F + k, (northToSouthDrawPos + 8.0F) * 0.00390625F + l)
                .color(s, t, u, layer.getCloudOpacity()).normal(0.0F, -1.0F, 0.0F).next();
            builder
                .vertex((double) (westToEastDrawPos + 8.0F + textureDisplacement), (double) (ab + 0.0F),
                    (double) (northToSouthDrawPos + 0.0F))
                .texture((westToEastDrawPos + 8.0F) * 0.00390625F + k, (northToSouthDrawPos + 0.0F) * 0.00390625F + l)
                .color(s, t, u, layer.getCloudOpacity()).normal(0.0F, -1.0F, 0.0F).next();
            builder
                .vertex((double) (westToEastDrawPos + 0.0F + textureDisplacement), (double) (ab + 0.0F),
                    (double) (northToSouthDrawPos + 0.0F))
                .texture((westToEastDrawPos + 0.0F) * 0.00390625F + k, (northToSouthDrawPos + 0.0F) * 0.00390625F + l)
                .color(s, t, u, layer.getCloudOpacity()).normal(0.0F, -1.0F, 0.0F).next();
          }
          ...
          if (ab <= 5.0F) {
            builder
                .vertex((double) (westToEastDrawPos + 0.0F + textureDisplacement),
                    (double) (ab + cloudThickness - 9.765625E-4F), (double) (northToSouthDrawPos + 8.0F))
                .texture((westToEastDrawPos + 0.0F) * 0.00390625F + k, (northToSouthDrawPos + 8.0F) * 0.00390625F + l)
                .color(m, n, o, layer.getCloudOpacity()).normal(0.0F, 1.0F, 0.0F).next();
            builder
                .vertex((double) (westToEastDrawPos + 8.0F + textureDisplacement),
                    (double) (ab + cloudThickness - 9.765625E-4F), (double) (northToSouthDrawPos + 8.0F))
                .texture((westToEastDrawPos + 8.0F) * 0.00390625F + k, (northToSouthDrawPos + 8.0F) * 0.00390625F + l)
                .color(m, n, o, layer.getCloudOpacity()).normal(0.0F, 1.0F, 0.0F).next();
            builder
                .vertex((double) (westToEastDrawPos + 8.0F + textureDisplacement),
                    (double) (ab + cloudThickness - 9.765625E-4F), (double) (northToSouthDrawPos + 0.0F))
                .texture((westToEastDrawPos + 8.0F) * 0.00390625F + k, (northToSouthDrawPos + 0.0F) * 0.00390625F + l)
                .color(m, n, o, layer.getCloudOpacity()).normal(0.0F, 1.0F, 0.0F).next();
            builder
                .vertex((double) (westToEastDrawPos + 0.0F + textureDisplacement),
          ...
          // * This is the only culling if sentence where we take displacement into
          // account.
          // * Since we are displacing all layers to the opposite direction (East) by
          // default.
          int ag;
          if (westToEastSpan > -1 - displacementInQuadrants) {
            for (ag = 0; ag < 8; ++ag) {
              builder
                  .vertex((double) (westToEastDrawPos + (float) ag + 0.0F + textureDisplacement), (double) (ab + 0.0F),
                      (double) (northToSouthDrawPos + 8.0F))
                  .texture((westToEastDrawPos + (float) ag + 0.5F) * 0.00390625F + k,
                      (northToSouthDrawPos + 8.0F) * 0.00390625F + l)
                  .color(p, q, r, layer.getCloudOpacity()).normal(-1.0F, 0.0F, 0.0F).next();
              builder
                  .vertex((double) (westToEastDrawPos + (float) ag + 0.0F + textureDisplacement),
                      (double) (ab + cloudThickness), (double) (northToSouthDrawPos + 8.0F))
                  .texture((westToEastDrawPos + (float) ag + 0.5F) * 0.00390625F + k,
                      (northToSouthDrawPos + 8.0F) * 0.00390625F + l)
                  .color(p, q, r, layer.getCloudOpacity()).normal(-1.0F, 0.0F, 0.0F).next();
              builder
                  .vertex((double) (westToEastDrawPos + (float) ag + 0.0F + textureDisplacement),
                      (double) (ab + cloudThickness), (double) (northToSouthDrawPos + 0.0F))
                  .texture((westToEastDrawPos + (float) ag + 0.5F) * 0.00390625F + k,
                      (northToSouthDrawPos + 0.0F) * 0.00390625F + l)
                  .color(p, q, r, layer.getCloudOpacity()).normal(-1.0F, 0.0F, 0.0F).next();
              builder
                  .vertex((double) (westToEastDrawPos + (float) ag + 0.0F + textureDisplacement), (double) (ab + 0.0F),
                      (double) (northToSouthDrawPos + 0.0F))
          ...
          if (westToEastSpan <= 1) {
            for (ag = 0; ag < 8; ++ag) {
              builder
                  .vertex((double) (westToEastDrawPos + (float) ag + 1.0F - 9.765625E-4F) + textureDisplacement,
                      (double) (ab + 0.0F), (double) (northToSouthDrawPos + 8.0F))
                  .texture((westToEastDrawPos + (float) ag + 0.5F) * 0.00390625F + k,
                      (northToSouthDrawPos + 8.0F) * 0.00390625F + l)
                  .color(p, q, r, layer.getCloudOpacity()).normal(1.0F, 0.0F, 0.0F).next();
              builder
              ...
              builder
                  .vertex((double) (westToEastDrawPos + (float) ag + 1.0F - 9.765625E-4F) + textureDisplacement,
                      (double) (ab + 0.0F), (double) (northToSouthDrawPos + 0.0F))
                  .texture((westToEastDrawPos + (float) ag + 0.5F) * 0.00390625F + k,
                      (northToSouthDrawPos + 0.0F) * 0.00390625F + l)
                  .color(p, q, r, layer.getCloudOpacity()).normal(1.0F, 0.0F, 0.0F).next();
            }
          ...
          if (northToSouthSpan > -1) {
            for (ag = 0; ag < 8; ++ag) {
              builder
                  .vertex((double) (westToEastDrawPos + 0.0F) + textureDisplacement, (double) (ab + cloudThickness),
                      (double) (northToSouthDrawPos + (float) ag + 0.0F))
                  .texture((westToEastDrawPos + 0.0F) * 0.00390625F + k,
                      (northToSouthDrawPos + (float) ag + 0.5F) * 0.00390625F + l)
                  .color(v, w, aa, layer.getCloudOpacity()).normal(0.0F, 0.0F, -1.0F).next();
              builder
                  .vertex((double) (westToEastDrawPos + 8.0F) + textureDisplacement, (double) (ab + cloudThickness),
                      (double) (northToSouthDrawPos + (float) ag + 0.0F))
                  .texture((westToEastDrawPos + 8.0F) * 0.00390625F + k,
                      (northToSouthDrawPos + (float) ag + 0.5F) * 0.00390625F + l)
                  .color(v, w, aa, layer.getCloudOpacity()).normal(0.0F, 0.0F, -1.0F).next();
              builder
                  .vertex((double) (westToEastDrawPos + 8.0F) + textureDisplacement, (double) (ab + 0.0F),
          ...
          if (northToSouthSpan <= 1) {
            for (ag = 0; ag < 8; ++ag) {
              builder
                  .vertex((double) (westToEastDrawPos + 0.0F) + textureDisplacement, (double) (ab + cloudThickness),
                      (double) (northToSouthDrawPos + (float) ag + 1.0F - 9.765625E-4F))
                  .texture((westToEastDrawPos + 0.0F) * 0.00390625F + k,
                      (northToSouthDrawPos + (float) ag + 0.5F) * 0.00390625F + l)
                  .color(v, w, aa, layer.getCloudOpacity()).normal(0.0F, 0.0F, 1.0F).next();
              builder
    ...
    } else if ((layer.getCloudType().equals(CloudConfig.CloudTypes.FAST)
        || layer.getCloudType().equals(CloudConfig.CloudTypes.LOD)) && layer.isWithinRenderDistance()) {
      for (westToEastSpan = northwestSpan - displacementInQuadrants; westToEastSpan <= southeastSpan
          - displacementInQuadrants; ++westToEastSpan) {
        for (northToSouthSpan = northwestSpan; northToSouthSpan <= southeastSpan; ++northToSouthSpan) {
          float westToEastDrawPos = (float) (westToEastSpan * 8);
          ...
          builder
              .vertex((double) (westToEastDrawPos + 0.0F + textureDisplacement), (double) (ab + 0.0F),
                  (double) (northToSouthDrawPos + 8.0F))
              .texture((float) (westToEastDrawPos + 0) * 0.00390625F + k,
                  (float) (northToSouthDrawPos + 8) * 0.00390625F + l)
              .color(m, n, o, 0.8F).normal(0.0F, -1.0F, 0.0F).next();
          builder
              .vertex((double) (westToEastDrawPos + 8.0F + textureDisplacement), (double) (ab + 0.0F),
                  (double) (northToSouthDrawPos + 8.0F))
              .texture((float) (westToEastDrawPos + 8) * 0.00390625F + k,
                  (float) (northToSouthDrawPos + 8) * 0.00390625F + l)
              .color(m, n, o, 0.8F).normal(0.0F, -1.0F, 0.0F).next();
          builder
              .vertex((double) (westToEastDrawPos + 8.0F + textureDisplacement), (double) (ab + 0.0F),
                  (double) (northToSouthDrawPos + 0.0F))
              .texture((float) (westToEastDrawPos + 8) * 0.00390625F + k,
                  (float) (northToSouthDrawPos + 0) * 0.00390625F + l)
              .color(m, n, o, 0.8F).normal(0.0F, -1.0F, 0.0F).next();
          builder
              .vertex((double) (westToEastDrawPos + 0.0F + textureDisplacement), (double) (ab + 0.0F),
                  (double) (northToSouthDrawPos + 0.0F))
